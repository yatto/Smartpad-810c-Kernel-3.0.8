...
int hdmi_status_suspend = 0;

extern int hdmi_loaded;
...
static int anx7150_remove(struct hdmi *hdmi)
{
	...
	if(!hdmi_status_suspend)
	{
		hdmi_set_backlight(HDMI_ENABLE);
	}
	printk("-->anx7150_remove!!\n");
	return 0;
}
...
static void anx7150_early_resume(struct early_suspend *h)
{
	...
	if((!gpio_get_value(anx->client->irq))&& (anx->hdmi->display_on == 1))
	{
		printk("--->HDMI is detected!!\n");
		hdmi_connected = 1;
	}
	else
		hdmi_connected = 0;
	hdmi_status_suspend = 0;
	hdmi_resume(anx->hdmi);
	return;
}
#endif
#if defined (GPIO_ANX7150_RST)
static void anx7150_hw_reset()
{
	int ret;

	if (ret = gpio_request(GPIO_ANX7150_RST, NULL)) {
...
#endif
	gpio_direction_output(GPIO_ANX7150_RST, GPIO_LOW);
	gpio_pull_updown(GPIO_ANX7150_RST, 1);
