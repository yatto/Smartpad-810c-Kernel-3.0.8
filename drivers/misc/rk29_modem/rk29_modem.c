...
static struct device *yf_modem_dev = NULL;

//static void rk29_modem_turnon(struct rk29_io_t *modem_power, int onoff);
static void rk29_modem_turnon(struct rk29_modem_t *rk29_modem, int onoff);
...
int rk29_modem_change_status(struct rk29_modem_t *rk29_modem, int status)
{
    ...
    case MODEM_DISABLE:
        ...
        rk29_modem_turnon(rk29_modem, 0);
        break;
    case MODEM_ENABLE :
        rk29_modem_turnon(rk29_modem, 1);
        break;
    case MODEM_SLEEP:
        ret = -1;
    case MODEM_WAKEUP:
...
static ssize_t modem_status_write(struct device *dev, struct device_attribute *attr, const char *_buf, size_t _count)
{
    ...
    if( !ret ) 
    {
		rk29_modem->status = new_state;
    }
    return _count;
}

static ssize_t modem_status_read(struct device *dev, struct device_attribute *attr, char *_buf)
{
    struct rk29_modem_t *rk29_modem = g_rk29_modem;
    return sprintf(_buf, "%d\n", rk29_modem->status);
}
static DEVICE_ATTR(modem_status, 0666, modem_status_read, modem_status_write);
...
static void rk29_modem_turnon(struct rk29_modem_t *rk29_modem, int onoff)
{
    if(rk29_modem->modem_power) {
        gpio_direction_output(rk29_modem->modem_power->io_addr, 
					onoff?rk29_modem->modem_power->enable:rk29_modem->modem_power->disable);
		msleep(100);
	}

    if(rk29_modem->modem_onoff) {
        gpio_direction_output(rk29_modem->modem_onoff->io_addr, 
					onoff?rk29_modem->modem_onoff->enable:rk29_modem->modem_onoff->disable);
		msleep(100);
	}

    if(rk29_modem->modem_reset) {
		if (onoff) {
			gpio_direction_output(rk29_modem->modem_reset->io_addr, 
					rk29_modem->modem_reset->enable);
			msleep(120);
			gpio_direction_output(rk29_modem->modem_reset->io_addr, 
					rk29_modem->modem_reset->disable);
		} else {
			gpio_direction_output(rk29_modem->modem_reset->io_addr, 
					rk29_modem->modem_reset->enable);
		}
...
// 申请控制modem电源的GPIO
    if( rk29_modem->modem_onoff )
    {
        ...
        if(ret != 0)
        {
            ...
            printk(">>>>>> Modem onoff io request failed!\n");
            return ret;
        }
    }
    ...
    if( rk29_modem->modem_reset)
    {
        ret = gpio_request(rk29_modem->modem_reset->io_addr, "modem_reset");
        if(ret != 0)
        {
            gpio_free(rk29_modem->modem_reset->io_addr);
            printk(">>>>>> Modem reset io request failed!\n");
            return ret;
        }
    }
...
int rk29_modem_init(struct rk29_modem_t *rk29_modem)
{
	...
	yf_modem_dev = device_create(rk29_modem_class, NULL, MKDEV(0, 1), NULL, "modem");
	ret = device_create_file(yf_modem_dev, &dev_attr_modem_status);
	if (ret)
	{
...
// 对modem初始化
	...
	printk("--> finished loaded modem driver\n");
    return 0;
