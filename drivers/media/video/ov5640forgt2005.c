...
;
	...
	printk("%s write sensor_WhiteB_Auto", __func__);
	ret = sensor_write_array(client, sensor_WhiteB_Auto);
	printk(" done\n");	
...
static ssize_t ov5640_show(struct class *cls, char *_buf)
{
	return printk("%s()\n", __FUNCTION__);
}
...
static ssize_t ov5640_store(struct class *cls, const char *_buf, size_t _count)
{
	const char * p=_buf;
	u16 reg;
	u8 val;

	if(!strncmp(_buf, "getall", strlen("getall")))
	{
		int i;
		printk("\n******************************\n");
		for(i=0; all_reg[i]; i++)
		{
			sensor_read(g_i2c_client, all_reg[i], &val);
...
static struct class *ov5640_class = NULL;
static CLASS_ATTR(ov5640, 0666, ov5640_show, ov5640_store);
			 ...
			 const struct i2c_device_id *did)
{
	...
	ov5640_class = class_create(THIS_MODULE, "ov5640");
	if (IS_ERR(ov5640_class)) 
	{
		printk("Create class ov5640.\n");
		return -ENOMEM;
	}
	class_create_file(ov5640_class,&class_attr_ov5640);
...
struct cam_sensor_info cam_sensor_info_ov5640forgt2005={
    "ov5640forgt2005",
    2,
    1,
    0x78>>1,
...
EXPORT_SYMBOL_GPL(cam_sensor_info_ov5640forgt2005);
