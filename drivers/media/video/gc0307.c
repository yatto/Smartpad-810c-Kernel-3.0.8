...
#define SENSOR_MAX_WIDTH    640//800//1600
#define SENSOR_MAX_HEIGHT   480//600//1200
...
//#define SENSOR_BUS_PARAM  (SOCAM_MASTER | SOCAM_PCLK_SAMPLE_RISING |\
//                          SOCAM_HSYNC_ACTIVE_LOW | SOCAM_VSYNC_ACTIVE_HIGH |\
//                          SOCAM_DATA_ACTIVE_HIGH | SOCAM_DATAWIDTH_8  |SOCAM_MCLK_24MHZ)
...
#ifndef GC0307_FLIP
#define GC0307_FLIP		0x00
#endif
#ifndef GC0307_MIRROR
#define GC0307_MIRROR	0x00
#endif
...
//========= close output
	...
	{0x42  ,0x10},					  	
	{0x47  ,0x00}, //mode1,				  	
	{0x48  ,0xc7},//0xc3}, //mode2, 	
	{0x49  ,0x00}, //dither_mode 		
	{0x4a  ,0x00}, //clock_gating_en
	{0x4b  ,0x00}, //mode_reg3
	{0x4E  ,0x20},//0x22},//0x21},//0x23},//0x22},//0x23}, //sync mode yaowei
	...
	{0x2a  ,0x02},//0x03}, //AEC_exp_level_2bit11to8   
	{0x2b  ,0x58},//0x84}, //AEC_exp_level_2bit7to0			 
	{0x2c  ,0x02},//0x09}, //AEC_exp_level_3bit11to8   659 - 8FPS,  8ca - 6FPS  //	 
	{0x2d  ,0x58},//0x60}, //AEC_exp_level_3bit7to0			 
	...
	{0xd6  ,0x4b},//0x96}, //antiflicker_step 					   
...
/////  请注意，调整GC0307的镜像和翻转，需要同时修改三个寄存器，如下:
	{0x0f, 0x82 & ~(0x03<<4) | (GC0307_FLIP<<5) | (GC0307_MIRROR<<4)},
	{0x45, 0x24 & ~(0x03<<0) | (GC0307_FLIP<<1) | (GC0307_MIRROR<<0)},
	{0x47, 0x20 & ~(0x03<<2) | (GC0307_FLIP<<3) | (GC0307_MIRROR<<2)},
///banding setting   
        {  0x01  ,0x6a}, // 24M  
        {  0x02  ,0x70}, 
        {  0x10  ,0x00},   
...
static int sensor_ioctrl(struct soc_camera_device *icd,enum rk29sensor_power_cmd cmd, int on)
{
	...
	switch (cmd)
	{
		...
		{
			printk("+++%s(): Sensor_PowerDown\n", __FUNCTION__);
			if (icl->powerdown) {
				ret = icl->powerdown(icd->pdev, on);
				gpio_set_value(CONFIG_SENSOR_POWERDN_PIN_1, on);
				gpio_direction_output(CONFIG_SENSOR_POWERDN_PIN_1, 0);
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
	...
	printk("*** sensor_init set powerdown\n");
	if (sensor_ioctrl(icd, Sensor_PowerDown, 0) < 0) {
		ret = -ENODEV;
		goto sensor_INIT_ERR;
	}
	gpio_set_value(CONFIG_SENSOR_POWERDN_PIN_1, 0);
	gpio_direction_output(CONFIG_SENSOR_POWERDN_PIN_1, 0);
...
static int sensor_deactivate(struct i2c_client *client)
{
	...
	gpio_set_value(CONFIG_SENSOR_POWERDN_PIN_1, 1);
	gpio_direction_output(CONFIG_SENSOR_POWERDN_PIN_1, 0);
			       ...
			       struct i2c_client *client)
{
	...
	printk("*** sensor_video_probe set powerdown\n");
	if (sensor_ioctrl(icd, Sensor_PowerDown, 0) < 0) {
		ret = -ENODEV;
...
static long sensor_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	switch (cmd)
	{
		...
		{
			printk("+++%s(): RK29_CAM_SUBDEV_DEACTIVATE\n", __FUNCTION__);
			sensor_deactivate(client);
			break;
		}
		...
		{
			printk("+++%s(): RK29_CAM_SUBDEV_IOREQUEST\n", __FUNCTION__);
...
static ssize_t gc0307_show(struct class *cls, char *_buf)
{
	return printk("%s()\n", __FUNCTION__);
}
...
static ssize_t gc0307_store(struct class *cls, const char *_buf, size_t _count)
{
	const char * p=_buf;
	u8 reg;
	u8 val;
...
static struct class *gc0307_class = NULL;
static CLASS_ATTR(gc0307, 0666, gc0307_show, gc0307_store);
			 ...
			 const struct i2c_device_id *did)
{
	...
	printk("*** %s module name is %s\n", __FUNCTION__, icl->module_name);
	...
	gc0307_class = class_create(THIS_MODULE, "gc0307");
	if (IS_ERR(gc0307_class)) 
	{
		printk("Create class gc0307.\n");
		return -ENOMEM;
	}
	class_create_file(gc0307_class,&class_attr_gc0307);
	g_i2c_client=client;
...
struct cam_sensor_info cam_sensor_info_gc0307={
    "gc0307",
    1,
    1,
    0x42>>1,
...
EXPORT_SYMBOL_GPL(cam_sensor_info_gc0307);
