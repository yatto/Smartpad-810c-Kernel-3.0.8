...
#define SENSOR_NAME gt2005_back
#define SENSOR_V4L2_IDENT_BACK  V4L2_IDENT_GT2005_BACK
...
#define MASK_GT2005_BACK_ORIENT	(0x01<<1 | 0x01<<0)
#ifndef GT2005_BACK_MIRROR
#define GT2005_BACK_MIRROR	0
#endif
#ifndef GT2005_BACK_FLIP
#define GT2005_BACK_FLIP	0
#endif
...
static u16 UXGA_Cap = 0;    //added by navy20110517
/* init 352X288 SVGA */
static struct reginfo sensor_init_data[] =
{
{0x0101 , GT2005_BACK_MIRROR<<0 | GT2005_BACK_FLIP<<1},//          0x01},  //  0 Õý³£  1  H¾µÏñ 2   V¾µÏñ  3   HV¾µÏñ
{0x0103 , 0x00}, 
{0x0105 , 0x00},
{0x0106 , 0xF0},
{0x0107 , 0x00},
...
//	gpio_direction_output(SENSOR_POWERDN_PIN_1, 0);
   ...
   /* ret = sensor_write(client, 0x3012, 0x80);
    ...
    if (pid == SENSOR_ID) {
        sensor->model = SENSOR_V4L2_IDENT_BACK;
    } else {
...
static int sensor_s_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	...
	if ((int)winseqe_set_addr  != sensor->info_priv.winseqe_cur_addr) {
		...
		sensor_write(client, 0x020B , 0x28);
		sensor_write(client, 0x020C , 0x44);

		sensor_write(client, 0x040A , 0x00);
		sensor_write(client, 0x040B , 0xf6);
		...
		sensor_write(client, 0x020B , 0x48);
		sensor_write(client, 0x020C , 0x64);

		sensor_write(client, 0x040A , 0x40);
		sensor_write(client, 0x040B , 0x33);
 ...
 static int sensor_g_chip_ident(struct v4l2_subdev *sd, struct v4l2_dbg_chip_ident *id)
{
    ...
    id->ident = SENSOR_V4L2_IDENT_BACK;      /* ddl@rock-chips.com :  Return gt2005  identifier */
    id->revision = 0;
...
static ssize_t gt2005_back_show(struct class *cls, char *_buf)
{
	return printk("%s()\n", __FUNCTION__);
}
...
static ssize_t gt2005_back_store(struct class *cls, const char *_buf, size_t _count)
{
	const char * p=_buf;
	u16 reg;
	u8 val;
...
static struct class *gt2005_back_class = NULL;
static CLASS_ATTR(gt2005_back, 0666, gt2005_back_show, gt2005_back_store);
			 ...
			 const struct i2c_device_id *did)
{
    ...
    gt2005_back_class = class_create(THIS_MODULE, "gt2005_back");
    if (IS_ERR(gt2005_back_class)) 
    {
        printk("Create class gt2005.\n");
        return -ENOMEM;
    }
    class_create_file(gt2005_back_class,&class_attr_gt2005_back);
...
struct cam_sensor_info cam_sensor_info_gt2005_back={
    "gt2005_back",
    2,
    1,
    0x78>>1,
...
EXPORT_SYMBOL_GPL(cam_sensor_info_gt2005_back);
