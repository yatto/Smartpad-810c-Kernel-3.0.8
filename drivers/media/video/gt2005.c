...
#endif
{
    ...
    if(!is_cam_active)
        return;
	sensor_read(client, 0x0101, &val);
	if( (!mode && (val&MASK_GT2005_ORIENT)!=0x01) || 
		(mode && (val&MASK_GT2005_ORIENT)!=0x02) ) {
		if(!mode && (val&MASK_GT2005_ORIENT)!=0x01) {
			printk("%s(): mode=%d, val=0x%02x, change cam orientation to horz\n", __FUNCTION__, mode, val);
			sensor_write(client, 0x0101, 0x01);
		}
		else if(mode && (val&MASK_GT2005_ORIENT)!=0x02) {
			printk("%s(): mode=%d, val=0x%02x, change cam orientation to vert\n", __FUNCTION__, mode, val);
			sensor_write(client, 0X0101,  0x02);
		}
	}
}
#ifdef CONFIG_SOC_CAMERA_FCAM
...
struct cam_sensor_info cam_sensor_info_gt2005={
    "gt2005",
    2,
    1,
    0x78>>1,
...
EXPORT_SYMBOL_GPL(cam_sensor_info_gt2005);
