...
#define EMPTY_ADVALUE					1000
#define DRIFT_ADVALUE					40
...
#define ADCKEY_BUFFER_COUNT     3
int adckey_vals[ADCKEY_BUFFER_COUNT];
int adckey_buffer_cnt = 0;
int adckey_nokey_cnt = 0;
static void callback(struct adc_client *client, void *client_param, int result)
{
	...
	key_dbg(ddata, "ADC key button->adc_value=%d\n", result);
	if(result < INVALID_ADVALUE || result > EMPTY_ADVALUE) 
	{
	    adckey_buffer_cnt = 0;
	    adckey_nokey_cnt++;
	    ddata->result = result;
	    if (adckey_nokey_cnt < ADCKEY_BUFFER_COUNT) 
	    {
	        return;
	    }
	}
	...
	{
		//printk("ADC key button->adc_value=%d\n", result);
		adckey_nokey_cnt = 0;
		adckey_vals[adckey_buffer_cnt % ADCKEY_BUFFER_COUNT] = result;
    	adckey_buffer_cnt++;
    	if (adckey_buffer_cnt < ADCKEY_BUFFER_COUNT) 
    	{
    	    return;
    	}
    	    
    	for (i = 0; i < ADCKEY_BUFFER_COUNT; i++)
    	{
    	    avradcval += adckey_vals[i];
    	}
    	avradcval = avradcval / ADCKEY_BUFFER_COUNT;
    	
    	for (i = 0; i < ADCKEY_BUFFER_COUNT; i++)
    	{
    	    if((adckey_vals[i] >= avradcval + DRIFT_ADVALUE) || (adckey_vals[i] <= avradcval - DRIFT_ADVALUE))
    	    {
    	        return;
    	    }
    	}
    ...
    for (i = 0; i < ddata->nbuttons; i++) 
	{
		...
		if(avradcval < button->adc_value + DRIFT_ADVALUE &&
			avradcval > button->adc_value - DRIFT_ADVALUE)
		{
			button->adc_state = 1;
			if (rk29_key_dbg_level)
			{
                printk("referrence = %d, ",button->adc_value);
                for (j = 0; j < ADCKEY_BUFFER_COUNT; j++)
            	{
            	    printk("buffer[%d] = %d, ",j,adckey_vals[j]);
            	}
            	printk("adcval = %d, result = %d\n", avradcval, result);
            }
		}
		else
		{
		...
		{
			mod_timer(&bdata->timer,jiffies + msecs_to_jiffies(DEFAULT_DEBOUNCE_INTERVAL));
		}		
	}
	return;
}
